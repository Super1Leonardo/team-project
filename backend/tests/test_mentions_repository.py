...
class DedupMentionsCursor:
    ...
    def __enter__(self) -> "DedupMentionsCursor":
        return self
...
class MentionDedupRepositoryTests(unittest.TestCase):
    def test_list_mentions_attaches_duplicates_for_dedup_groups(self) -> None:
        fake_cursor = DedupMentionsCursor()
        store = BrandRadarPostgresStore(Settings())
        store._connect = lambda *args, **kwargs: FakeConnection(fake_cursor)  # type: ignore[method-assign]
        ...
        self.assertEqual(mention["dedup_group_id"], 77)
        self.assertEqual(
            mention["dedup"],
            {
                "duplicates": [
                    {
                        ...
                        "is_primary": False,
                        "resolved": False,
                        "processed_at": fake_cursor._duplicate_rows[1]["processed_at"],
                        "source_id": 10,
                        "source_type": "rss",
                        "external_id": "x-2",
                        "url": "https://example.com/x-2",
                        ...
                        "text": "Duplicate body",
                        "author": "author-2",
                        "published_at": fake_cursor._duplicate_rows[1]["published_at"],
                        "collected_at": fake_cursor._duplicate_rows[1]["collected_at"],
                    }
                ]
            },
        )
        ...
        duplicates_query, duplicates_params = fake_cursor.executed[1]
        self.assertIn("m.dedup_group_id = ANY(%s)", duplicates_query)
        self.assertNotIn("m.is_primary = TRUE", duplicates_query)
        self.assertIn("m.relevance_label = 'relevant'", duplicates_query)
        self.assertEqual(duplicates_params, [3, [77], 0.7, "negative"])
