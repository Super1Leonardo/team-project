...
async function getOrCreateProject(): Promise<Project | null> {
  ...
  if (!projects || projects.length === 0) {
    const newProject = await fetchJson<Project>(
...
export const actions: Actions = {
  updateProject: async ({ request }) => {
    ...
    const keywords = keywordsStr.split(",").map((k) => k.trim()).filter((k) =>
      k
    );
    const exclude_keywords = excludeKeywordsStr.split(",").map((k) => k.trim())
      .filter((k) => k);
    const risk_words = riskWordsStr.split(",").map((k) => k.trim()).filter(
      (k) => k
    );
    ...
    return {
      success: true,
      message: "Настройки сохранены. Изменения войдут через минуту.",
      project: result,
    };
  },
};
